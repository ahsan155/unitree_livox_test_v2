...
class SinusoidalEmbeddingLayer(tf.keras.layers.Layer):
  ...
  def __init__(self, min_freq=4, max_freq=256, hidden_size=256, **kwargs):
    ...
    self.max_freq = float(max_freq)
    self.hidden_size = hidden_size
    self.num_freqs = hidden_size // 2

  def build(self, input_shape):
    # compute the inv_freqs as a numpy array (or tf.Tensor) once:
    log_increment = (
      tf.math.log(self.max_freq / self.min_freq) /
      tf.maximum(1.0, tf.cast(self.num_freqs - 1, tf.float32))
    )
    inv = self.min_freq * tf.exp(
      tf.cast(tf.range(self.num_freqs), tf.float32) * -log_increment
    )
    # register it as a non-trainable weight so it participates in the graph:
    self.inv_freqs = self.add_weight(
      name='inv_freqs',
      shape=(self.num_freqs,),
      initializer=tf.constant_initializer(inv.numpy()),
      trainable=False
    )
  ...
  def call(self, x):
    # x: [..., 1] or [..., 3], etc.
    # first broadcast so that x has a last dim = num_freqs
    x_expanded = tf.repeat(x[..., tf.newaxis], self.num_freqs, axis=-1)
    # apply sin and cos with the registered inv_freqs
    sin_emb = tf.sin(x_expanded * self.inv_freqs)
    cos_emb = tf.cos(x_expanded * self.inv_freqs)
    return tf.concat([sin_emb, cos_emb], axis=-1)
